...
- `font-family` 為所選的元素指定一個或者多個字體系列的優先列表
  ...
  - `selector 兄弟、後代、子代、子孫...之類`
    
    姑且分為 兩兄弟  ( \~ 和 +)   /   兩子代 ( > 和 " " )
    
    - ` " " ` 代表 空格 使用 **隔** 記憶，隔好幾層子孫或兄弟都算
    
    - ` > `  代表  子類   > 標籤右邊的意思 ， 加上沒有空格，  所以 就是標籤框起來的範圍，被直接接觸的子標籤都可以。
    
    - ` ~ `  代表 兄弟選擇器， \<p> \<a id='1'> \<a id='2'> \</p> ， p~a 則 1跟2都選擇
      
      ~ 波浪代表 很遠，再加上因為沒空格，所以是兄弟。
    ...
    - ` + `  代表  相鄰才能，因為沒空格所以 也就是  `h1 + p`  只有 p直接緊隨h2後的才能選，同時滿足 h2 跟 直接接觸 h2的 p。
